...
from autogen_magentic_one.queries.queries import queries_no_formatting_instructions, create_instructions
...
def generate_folder_name(difficulty, id_query):
...
###
async def main(logs_dir: str, hil_mode: bool, save_screenshots: bool, difficulty: str, id_query: int, num_listings: int, num_pages: int) -> None:
    ...
    async with DockerCommandLineCodeExecutor(work_dir=logs_dir) as code_executor:
        ...
        agent_list = [web_surfer, coder, executor]
        if hil_mode:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_listings", type=none_or_int, required=False, help="Number of listings to query (or None)")
    parser.add_argument("--num_pages", type=none_or_int, required=False, help="Number of websites to query (or None)")
    ...
    query = available_queries[id_query]
    num_listings = args.num_listings
    num_pages = args.num_pages
    instructions = create_instructions(num_listings, num_pages)
    final_query = query + instructions
